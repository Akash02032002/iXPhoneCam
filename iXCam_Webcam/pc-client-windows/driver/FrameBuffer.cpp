...
bool FrameBuffer::createWriter(int width, int height) {
    m_isWriter = true;
    m_width = width;
    m_height = height;
    ...
    printf("FrameBuffer: Writer created %dx%d, shared mem size=%d\n",
           m_width, m_height, m_totalSize);
    return true;
}
...
bool FrameBuffer::openReader() {
    ...
    printf("FrameBuffer: Reader opened %dx%d\n", m_width, m_height);
    return true;
}
