...
class StreamingService : Service() {
    ...
    companion object {
        ...
        private const val SEND_QUEUE_CAPACITY = 30
        private const val SEND_QUEUE_POLL_MS = 500L
    ...
    private val phoneSpeakerPlayer by lazy { PhoneSpeakerPlayer() }
    ...
    // Unified send queue - ALL outgoing data goes through this single queue
    // This prevents lock contention between video, audio, and heartbeat threads
    ...
    private val sendQueue = ArrayBlockingQueue<SendPacket>(SEND_QUEUE_CAPACITY)
    ...
    private fun handleClientConnected(client: TransportClient) {
        ...
        controlListenerThread = Thread({
            ...
            // Only disconnect if THIS client is still the active one
            if (connectedClient.get() === currentClient) {
                handleClientDisconnected()
            }
    ...
    private fun enqueueAudio(data: ByteArray, size: Int) {
        ...
        sendQueue.offer(packet) // Drop if queue full (audio loss is acceptable)
    }

    /**
     * Unified sender thread that reads from the send queue and writes to the socket.
     * ALL outgoing data flows through this single thread — no lock contention.
     */
    private fun startSenderThread() {
        senderThread = Thread({
            ...
            while (isStreaming.get()) {
                try {
                    val packet = sendQueue.poll(SEND_QUEUE_POLL_MS, TimeUnit.MILLISECONDS)
                        ?: continue
                    ...
                    when (packet) {
                        is SendPacket.Video -> {
                            ...
                            if (sent % 100 == 0L) {
                                Log.i(TAG, "Sender: video #$sent (${packet.data.size}B, key=${packet.isKeyFrame}, q=${sendQueue.size})")
                            }
        ...
        heartbeatThread = Thread({
            while (isStreaming.get()) {
                try {
                    ...
                    sendQueue.offer(SendPacket.Heartbeat) // Non-blocking, skip if queue full
                } catch (e: InterruptedException) {
                    break
                }
            }
